...
DEFAULT_FALLBACK_PROBABILITY = AUTOPLAY_FALLBACK_PROBABILITY
DEFAULT_OUTPUT_TEMPLATE = "autoplay_graph_{folder}.html"
DEFAULT_PAIRING_MODE = "window"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "'consecutive' only links adjacent plays."
        ),
    )
    parser.add_argument(
        ...
        help=(
            ...
            f"(default: {DEFAULT_FALLBACK_PROBABILITY:.2f})"
        ),
    )
    parser.add_argument(
        "--include-self-loops",
...
def build_graph_payload(
    ...
    fallback_probability = clamp_probability(fallback_probability)
    ...
    for song_name in song_names:
        ...
        if observed_outgoing_count > 0:
            fallback_total_probability = fallback_probability if missing_neighbor_count > 0 else 0.0
            observed_total_probability = 1.0 - fallback_total_probability
        else:
            fallback_total_probability = 1.0 if missing_neighbor_count > 0 else 0.0
            observed_total_probability = 0.0

        fallback_per_song_probability = (
            fallback_total_probability / missing_neighbor_count if missing_neighbor_count > 0 else 0.0
        )
    ...
    for (source_song, target_song), count in edge_counts.items():
        ...
        missing_neighbor_count = max(0, total_song_count - 1 - len(outgoing_neighbors[source_song] - {source_song}))
        fallback_total_probability = fallback_probability if observed_outgoing_count > 0 and missing_neighbor_count > 0 else 0.0
        observed_total_probability = 1.0 - fallback_total_probability if observed_outgoing_count > 0 else 0.0
        probability = (observed_total_probability * count / observed_outgoing_count) if observed_outgoing_count > 0 else 0.0
    ...
    payload = {
        "summary": {
            ...
            "fallbackProbability": fallback_probability,
            "generatedFrom": META_FILENAME,
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .network-panel {{
      ...
      min-height: 0;
      overflow: hidden;
      background: radial-gradient(circle at top, rgba(56, 189, 248, 0.06), transparent 35%), var(--bg);
    }}
    #network {{
      width: 100%;
      height: 100%;
...
<body>
    ...
    <aside class=\"sidebar\">
      <section class=\"card\">
        ...
        <div style=\"margin-top: 12px;\">
          ...
          <span class=\"pill\" id=\"summary-fallback\"></span>
        </div>
      </section>
      <section class=\"card\">
        <h3>Selection</h3>
  ...
  <script>
    ...
    const nodeDataSet = new vis.DataSet(allNodes.map((node) => ({{
      ...node,
      font: {{ color: '#e5e7eb', size: 18, face: 'Inter' }},
      color: {{
        background: '#2563eb',
        border: '#7dd3fc',
        highlight: {{ background: '#7c3aed', border: '#c4b5fd' }}
      }}
    }})));
    ...
    const network = new vis.Network(container, {{ nodes: nodeDataSet, edges: edgeDataSet }}, {{
      ...
      edges: {{
        ...
        smooth: {{ type: 'dynamic' }},
        color: {{ color: 'rgba(148, 163, 184, 0.55)', highlight: '#f472b6', hover: '#f9a8d4' }},
        font: {{ color: '#cbd5e1', strokeWidth: 0, size: 11, align: 'top' }},
      }},
    ...
    function refreshGraph() {{
      ...
      nodeDataSet.add(allNodes
        ...
        .map((node) => ({{
          ...
          font: {{ color: '#e5e7eb', size: 18, face: 'Inter' }},
          color: {{
            background: node.id === selectedNodeId ? '#9333ea' : '#2563eb',
            border: node.id === selectedNodeId ? '#ddd6fe' : '#7dd3fc',
            highlight: {{ background: '#7c3aed', border: '#c4b5fd' }}
          }}
        }})));
      edgeDataSet.clear();
    ...
    function showNodeDetails(nodeId) {{
      ...
      document.getElementById('selection-content').innerHTML = `
        ...
        <div style=\"margin-top: 12px;\" class=\"muted\">
          Fallback pool: ${{formatPercent(node.fallbackTotalProbability)}} total ·
          ${{formatPercent(node.fallbackPerSongProbability)}} per unseen song
        </div>
    ...
    document.getElementById('summary-pairing').textContent = `Pairing: ${{graphData.summary.pairingMode}}`;
    document.getElementById('summary-fallback').textContent = `Fallback: ${{(graphData.summary.fallbackProbability * 100).toFixed(1)}}%`;
...
def main() -> int:
    ...
    output_path = Path(args.output).expanduser() if args.output else get_default_output_path(folder)
