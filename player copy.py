...
class CursesTUI:
    ...
    def _render_grid_table(self, grid_top_y: int, headers: Tuple[str, str, str],
                           rows: List[Tuple[str, str, str]],
                           numeric_is_float: bool) -> int:
        """
        ...
        if numeric_is_float:
            ...
            # compute width to fit the largest numeric string
            max_num_len = 1
        ...
        track_w = max(10, max_grid_w - (idx_w + num_w + 8))  # 8 = borders+seps and padding space
        if track_w < 5:
            track_w = 5

        # Determine how many rows fit vertically:
        # Table lines used for R visible rows = 2*R + 3 (top, header, header-sep, row+sep..., bottom)
        lines_avail = h - grid_top_y - 3  # leave 2 lines for input and 1 footer
        if lines_avail < 4:
            return grid_top_y  # too small to draw
        max_visible_rows = (lines_avail - 3) // 2
        max_visible_rows = max(1, max_visible_rows)  # at least 1 row
        ...
        # Top border
        self._draw_line(grid_top_y, indent, f"{TL}{h1}{TJ}{h2}{TJ}{h3}{TR}")

        # Header line
        h_idx, h_track, h_num = headers
        hdr = (
            f"{V} "
            f"{h_idx.center(idx_w)}"
            f" {V} "
            f"{h_track.ljust(track_w)}"
            f" {V} "
            f"{h_num.center(num_w)}"
            f" {V}"
        )
        self._draw_line(grid_top_y + 1, indent, hdr)

        # Header separator
        self._draw_line(grid_top_y + 2, indent, f"{LJ}{h1}{MJ}{h2}{MJ}{h3}{RJ}")
        ...
        for i, (idx_str, name, num_str) in enumerate(visible_rows):
            row = (
                f"{V} "
                f"{idx_str.rjust(idx_w)}"
                f" {V} "
                f"{name.ljust(track_w)[:track_w]}"
                f" {V} "
                f"{num_str.rjust(num_w)}"
                f" {V}"
            )
            self._draw_line(y, indent, row)
            y += 1
            # Row separator if not last visible row
            if i != len(visible_rows) - 1:
                self._draw_line(y, indent, f"{LJ}{h1}{MJ}{h2}{MJ}{h3}{RJ}")
                y += 1

        # Bottom border
        self._draw_line(y, indent, f"{BL}{h1}{BJ}{h2}{BJ}{h3}{BR}")
        return y + 1
    ...
    def _render_table_most(self, top_y: int, counts: Dict[str, int]) -> int:
        ...
        grid_top = top_y + 1
        next_y = self._render_grid_table(grid_top, headers=("#", "Track", "Plays"), rows=rows, numeric_is_float=False)
        return next_y

    def _render_table_similar(self, top_y: int,
                              similar_entries: Optional[List[Tuple[str, float]]],
                              mood: Optional[str]) -> int:
        ...
        grid_top = top_y + 1
        next_y = self._render_grid_table(grid_top, headers=("#", "Track", "Sim"), rows=rows, numeric_is_float=True)
        return next_y
...
def _canonicalize_tags(tags):
    ...
    return ", ".join(sorted({str(t).strip().lower() for t in (tags or []) if str(t).strip()}))
...
def ensure_embeddings(model, tags_data: dict, mp3_folder: str, logging: bool = False):
    ...
    if (os.path.exists(cache_file)
        and _EMB_MATRIX is None):  # only load once per process
        try:
            blob = np.load(cache_file, allow_pickle=True)
            if (str(blob.get("version", "")) == EMB_CACHE_VERSION
                and str(blob.get("model_name", "")) == getattr(model, "model_card", None) or True
                and str(blob.get("fingerprint", "")) == fp_now):
                names = list(blob["names"])
                embs = blob["embeddings"]
                # Basic sanity checks
                if isinstance(names, list) and isinstance(embs, np.ndarray) and embs.ndim == 2:
                    _EMB_NAMES = names
                    _EMB_MATRIX = embs
                    ...
                    if logging:
                        print(f"[emb] loaded {_EMB_MATRIX.shape[0]} track embeddings from cache.")
                    return
        except Exception as e:
            if logging:
    ...
    emb = model.encode(texts, batch_size=8, show_progress_bar=logging, convert_to_numpy=True)
    ...
    _EMB_MATRIX = emb
    _EMB_INDEX = {b: i for i, b in enumerate(_EMB_NAMES)}
    _EMB_MODEL_NAME = getattr(model, "model_card", None) or getattr(model, "model_name", None) or "unknown"
...
def compute_top_for_mood(model: SentenceTransformer,
                         ...
                         mp3_folder: str,
                         top_n: int,
                         logging: bool = False) -> Tuple[List[str], List[Tuple[str, float]]]:
    """
    FAST VERSION:
      ...
      - Preserves your exact/substring tag preference.
    Returns:
    ...
    q = model.encode(["music that is " + mood], batch_size=1, show_progress_bar=False, convert_to_numpy=True)[0]
    ...
    # Preference: names whose tags contain the exact or substring match of the mood term
    q_norm = mood.strip().lower()
    exact_like = []
    for base in _EMB_NAMES:
        tags = [str(t).lower() for t in (tags_data.get(base, []) or [])]
        exact_like.append(any(q_norm == t for t in tags) or any(q_norm in t for t in tags))

    # Rank: exact/substr first, then by similarity desc
    idxs = list(range(len(_EMB_NAMES)))
    idxs.sort(key=lambda i: (not exact_like[i], -float(sims[i])))

    # Clamp top-N (your main loop already clamps externally; this is a safety)
...
def get_audio_stats(full_filepath_with_extension: str) -> dict:
    ...
    try:
        audio = AudioSegment.from_mp3(full_filepath_with_extension)
        dur = float(audio.duration_seconds) if audio.duration_seconds and audio.duration_seconds > 0 else 0.0
        loud = float(audio.dBFS)
        if loud == -math.inf:
            loud = -100.0
        return {"loudness_dbfs": loud, "duration": dur, "mtime": os.path.getmtime(full_filepath_with_extension)}
    except CouldntDecodeError:
        return {"loudness_dbfs": None, "duration": 0.0, "mtime": os.path.getmtime(full_filepath_with_extension)}
    except Exception:
        return {"loudness_dbfs": None, "duration": 0.0, "mtime": os.path.getmtime(full_filepath_with_extension)}
...
def build_audio_data_for_playlist(playlist: List[str],
    ...
    for full_path in playlist:
        ...
        if lufs is None:
            audio_data[full_path] = {'loudness_dbfs': None, 'scale': 0.5, 'duration': dur}
        else:
            scale = calculate_volume_scale(resolved_target, float(lufs))
            audio_data[full_path] = {'loudness_dbfs': float(lufs), 'scale': float(scale), 'duration': dur}

    if logging:
        print(f"[loud] target={resolved_target:.2f} dBFS; analyzed {len(playlist)} files (cached={sum(1 for p in playlist if p in loudness_cache)}).")

    return resolved_target, audio_data

# ----------------- Inline command parsing -----------------

TOP_DIR_RE = re.compile(r"\(\s*top\s*[:=]?\s*(\d+)\s*\)|\btop\s*[:=]?\s*(\d+)\b", re.IGNORECASE)
...
def main(initial_mood: Optional[str],
    ...
    if initial_mood:
        pl, sim_entries = compute_top_for_mood(model, tags_data, initial_mood, mp3_folder_path, current_top_n, logging=logging)
        if pl:
            ...
            if not enable_tui:
                ...
                table_data = [[i+1, base, f"{score:.4f}"] for i, (base, score) in enumerate(sim_entries)]
                print("\n--- Top Mood Matches ---")
            ...
            while True:
                ...
                if not current_playlist:
                    ...
                    if submitted:
                        ...
                        if mood_text:
                            try:
                                pl, sim_entries = compute_top_for_mood(model, tags_data, mood_text, mp3_folder_path, current_top_n, logging=logging)
                                if pl:
                                    resolved_target_loudness, audio_data = build_audio_data_for_playlist(
                                        pl, target_lufs, mp3_folder_path, sample_file_name, loudness_cache, logging=logging
                                    )
                ...
                for full_path in round_list:
                    ...
                    if pending_mood:
                        try:
                            pl, sim_entries = compute_top_for_mood(model, tags_data, pending_mood, mp3_folder_path, current_top_n, logging=logging)
                            if pl:
                                resolved_target_loudness, audio_data = build_audio_data_for_playlist(
                                    pl, target_lufs, mp3_folder_path, sample_file_name, loudness_cache, logging=logging
                                )
                                ...
                                tui.view_mode = "similar"  # jump to Similar to show fresh table
                    ...
                    try:
                        ...
                        if user_stop:
                            ...
                            lock_since_wall = _sleep_with_exit_checks(0.2, next_exit_dt, lock_since_wall)
                            break
...
if __name__ == "__main__":
    ...
    parser.add_argument('-top', '--top', type=int, default=1,
