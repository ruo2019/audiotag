...
DEFAULT_SPEAKER_MIRROR_OFFSET_SEC = 0.085
MIN_SPEAKER_MIRROR_OFFSET_SEC = -0.250
MAX_SPEAKER_MIRROR_OFFSET_SEC = 0.250
SPEAKER_MIRROR_PREROLL_SEC = 0.180
...
SPEAKER_DIR_RE = re.compile(
    r"\(\s*speaker\s*[:=]?\s*(on|off)\s*\)"
    r"|\bspeaker\s*[:=]?\s*(on|off)\b",
    re.IGNORECASE,
)

SPEAKER_OFFSET_DIR_RE = re.compile(
    r"\(\s*speaker\s+(?:offset|delay|lag)\s*[:=]?\s*(-?\d+(?:\.\d+)?)\s*(ms|s)?\s*\)"
    r"|\bspeaker\s+(?:offset|delay|lag)\s*[:=]?\s*(-?\d+(?:\.\d+)?)\s*(ms|s)?\b",
    re.IGNORECASE,
)
...
def clamp_float(n: float, lo: float, hi: float) -> float:
...
def parse_mood_and_directives(
    ...
    speaker_val: Optional[bool] = None
    speaker_offset_val: Optional[float] = None
    ...
    for m in VOL_DIR_RE.finditer(raw):
        ...
        if num is not None:
            try:
                ...
                pass
    for m in SPEAKER_DIR_RE.finditer(raw):
        val = (m.group(1) or m.group(2) or "").strip().lower()
        if val == "on":
            speaker_val = True
        elif val == "off":
            speaker_val = False
    for m in SPEAKER_OFFSET_DIR_RE.finditer(raw):
        ...
        try:
            ...
            continue
        if unit == "s":
            speaker_offset_val = parsed
        elif unit == "ms":
            speaker_offset_val = parsed / 1000.0
        elif abs(parsed) < 1.0:
            speaker_offset_val = parsed
        else:
            speaker_offset_val = parsed / 1000.0
    ...
    mood = SPEAKER_DIR_RE.sub("", mood).strip()
    mood = SPEAKER_OFFSET_DIR_RE.sub("", mood).strip()
    mood = mood.strip(" ,;:()[]{}-") or None
    return mood, top_val, vol_val, speaker_val, speaker_offset_val
...
class CursesTUI:
    ...
    Minimal curses UI:
      ...
      - Mood bar also accepts "vol ..." and "speaker on/off"
    ...
    def render(
        ...
        sp = "on" if speaker_enabled else "off"
        so = f"{speaker_offset_sec * 1000.0:+.0f}ms"

        self._draw(2, 2, f"Target LUFS: {td}")
        self._draw(3, 2, f"Current LUFS: {cd}")
        self._draw(4, 2, f"LUFS Diff: {ld}")
        self._draw(5, 2, f"Adjusted Volume: {vs} | Speaker Mirror: {sp} ({so})")
        ...
        if self.show_help:
            help_lines = [
                ...
                "",
                "Enter: submit mood",
                "Mood directives: top=5, vol=-16, speaker on/off, speaker offset=85ms",
...
class PromptToolkitTUI(CursesTUI):
    """prompt_toolkit implementation with the same public flags as CursesTUI."""

    def __init__(self, enable: bool = True):
        super().__init__(enable=enable)
        self._lock = threading.RLock()
        self._submitted: "queue_mod.Queue[str]" = queue_mod.Queue()
        self._app: Optional[Application] = None
        self._thread: Optional[threading.Thread] = None
        self._closed = threading.Event()
        self._thread_error: Optional[BaseException] = None
        self._wheel_pending_panel: Optional[str] = None
        self._wheel_pending_direction = 0
        self._snapshot: Dict[str, object] = {
            ...
            "speaker_offset_sec": 0.0,
            "elapsed_sec": 0.0,
            "total_sec": 0.0,
            "playback_mode": "manual",
            "counts": {},
            "listen_hours_for_stem": None,
            "similar_entries": [],
            "similar_mood": None,
            "queue_items": [],
            "playlists": [],
    ...
    def __enter__(self):
        ...
        body = FormattedTextControl(
            text=self._build_fragments,
            focusable=True,
            key_bindings=self._build_key_bindings(),
            show_cursor=False,
        )
        self._app = Application(
            layout=Layout(Window(content=body, wrap_lines=False)),
            full_screen=True,
            mouse_support=True,
        ...
        def _run() -> None:
            try:
                ...
                self._app.run(set_exception_handler=False, handle_sigint=False)
            except BaseException as e:
                if not self._closed.is_set():
                    self._thread_error = e

        self._thread = threading.Thread(target=_run, name="prompt-toolkit-tui", daemon=True)
        self._thread.start()
        return self
    ...
    def _panel_len(self, panel: str) -> int:
        if panel == "queue":
            return self.queue_len
        if panel == "similar":
            return self.similar_len
        if panel == "playlists":
            return self.playlist_len
        return self.most_len

    def _set_selected(self, panel: str, value: int) -> None:
        n = self._panel_len(panel)
        value = max(0, min(max(0, n - 1), value))
        if panel == "queue":
            self.queue_selected = value
        elif panel == "similar":
            self.similar_selected = value
        elif panel == "playlists":
            self.playlist_selected = value
        else:
            self.most_selected = value

    def _selected(self, panel: str) -> int:
        if panel == "queue":
            return self.queue_selected
        if panel == "similar":
            return self.similar_selected
        if panel == "playlists":
            return self.playlist_selected
        return self.most_selected

    def _move_selection(self, amount: int) -> None:
        with self._lock:
            panel = self.focus_panel
            self._set_selected(panel, self._selected(panel) + amount)
        self._invalidate()

    def _page_amount(self) -> int:
        return max(1, self.last_h - 12)
    ...
    def _cycle_focus(self, direction: int) -> None:
        ...
        with self._lock:
            try:
                ...
                idx = 0
            self.focus_panel = order[(idx + direction) % len(order)]
        self._invalidate()
    ...
    def _handle_wheel(self, panel: str, direction: int) -> None:
        self._wheel_pending_panel = panel
        self._wheel_pending_direction = direction

    def _consume_pending_wheel(self) -> None:
        panel = self._wheel_pending_panel
        direction = self._wheel_pending_direction
        self._wheel_pending_panel = None
        self._wheel_pending_direction = 0
        if panel is None or not direction:
            return
        self.focus_panel = panel
        self._set_selected(panel, self._selected(panel) + direction)
    ...
    def _build_key_bindings(self) -> KeyBindings:
        ...
        def _(event) -> None:
            self._move_selection(-self._page_amount())
        ...
        def _(event) -> None:
            self._submit_or_activate()

        @kb.add("<any>")
        def _(event) -> None:
            data = event.data
            if data and len(data) == 1 and 32 <= ord(data) <= 126:
                with self._lock:
    ...
    def _mouse_handler(self, panel: str, row_idx: Optional[int] = None, role: str = "row"):
        def _handler(mouse_event):
            event_type = mouse_event.event_type
            x = getattr(mouse_event.position, "x", 0)
            with self._lock:
                if event_type == MouseEventType.SCROLL_UP:
                    self._handle_wheel(panel, -1)
                    return None
                if event_type == MouseEventType.SCROLL_DOWN:
                    self._handle_wheel(panel, 1)
                    return None
                if panel == "queue" and row_idx is not None:
                    if event_type == MouseEventType.MOUSE_MOVE and self.queue_drag_start is not None:
                        self.queue_drag_target = row_idx
                        self.queue_selected = min(max(0, self.queue_len - 1), row_idx)
                        self._invalidate()
                        return None
                    if event_type == MouseEventType.MOUSE_UP and self.queue_drag_start is not None:
                        start_idx = self.queue_drag_start
                        self.queue_drag_target = row_idx
                        self.queue_selected = min(max(0, self.queue_len - 1), row_idx)
                        if row_idx == start_idx:
                            self.queue_click_row = row_idx
                            self.queue_click_x = x
                        else:
                            self.queue_drag_commit_start = start_idx
                            self.queue_drag_commit_target = row_idx
                        self.queue_drag_start = None
                        self._invalidate()
                        return None
                ...
                if role == "mode":
                    ...
                    return None
                if role == "most_title":
                    if self.most_sort_col_start <= x < self.most_sort_col_end:
                        self.most_toggle_request = True
                    else:
                        self.focus_panel = "most"
                    self._invalidate()
                ...
                self.focus_panel = panel
                self._set_selected(panel, row_idx)
                now = time.monotonic()
                is_double = (
                    self._last_click_panel == panel
                    and self._last_click_row == row_idx
                    and (now - self._last_click_ts) <= 0.35
                )

                if panel == "queue":
                    self.queue_drag_start = row_idx
                    self.queue_drag_target = row_idx
                    self.queue_drag_commit_start = row_idx
                    self.queue_drag_commit_target = None
                    if is_double and self.queue_name_col_start <= x < self.queue_name_col_end:
                        self.queue_delete = True
                elif panel == "similar" and is_double:
                    self.similar_add = True
                elif panel == "playlists" and is_double:
                    self.playlist_activate = True
                elif panel == "most" and is_double:
                    self.most_add = True
    ...
    def _terminal_size(self) -> Tuple[int, int]:
        if self._app is not None:
            try:
                ...
                return max(10, size.rows), max(40, size.columns)
            except Exception:
                pass
    ...
    def _line(self, fragments: List[tuple], text: str, width: int, style: str = "", handler=None) -> None:
        fragments.append((style, self._fit(text, width), handler) if handler else (style, self._fit(text, width)))
        fragments.append(("", "\n"))
    ...
    def _build_fragments(self):
        ...
        now_playing = str(snap.get("now_playing") or "")
        target_lufs = snap.get("target_lufs")
        current_lufs = snap.get("current_lufs")
        loudness_diff = snap.get("loudness_diff")
        volume_scale = snap.get("volume_scale")
        speaker_enabled = bool(snap.get("speaker_enabled"))
        speaker_offset_sec = float(snap.get("speaker_offset_sec") or 0.0)
        elapsed_sec = float(snap.get("elapsed_sec") or 0.0)
        total_sec = float(snap.get("total_sec") or 0.0)
        playback_mode = str(snap.get("playback_mode") or "manual")
        counts = dict(snap.get("counts") or {})
        listen_hours_for_stem = snap.get("listen_hours_for_stem")
        similar_entries = list(snap.get("similar_entries") or [])
        similar_mood = snap.get("similar_mood")
        queue_items = list(snap.get("queue_items") or [])
        ...
        with self._lock:
            ...
            self.playback_mode_col_end = chip_x + len(mode_chip)
            self.playback_mode = playback_mode
        fragments.append(("", self._fit(header[:chip_x], chip_x)))
        fragments.append(("class:focus", mode_chip, self._mouse_handler("queue", role="mode")))
        fragments.append(("", self._fit("", max(0, w - chip_x - len(mode_chip)))))
        fragments.append(("", "\n"))

        self._line(fragments, f"  Track: {now_playing or '(none)'}", w)
        td = "N/A" if target_lufs is None else f"{float(target_lufs):.2f}"
        cd = "N/A" if current_lufs is None else f"{float(current_lufs):.2f}"
        ld = "N/A" if loudness_diff is None else f"{float(loudness_diff):.2f}"
        vs = "N/A" if volume_scale is None else f"{float(volume_scale):.2f}"
        sp = "on" if speaker_enabled else "off"
        so = f"{speaker_offset_sec * 1000.0:+.0f}ms"
        self._line(fragments, f"  Target LUFS: {td}", w)
        self._line(fragments, f"  Current LUFS: {cd}", w)
        self._line(fragments, f"  LUFS Diff: {ld}", w)
        self._line(fragments, f"  Adjusted Volume: {vs} | Speaker Mirror: {sp} ({so})", w)
        ...
        self._line(
            fragments,
            f"  {bar} {int(elapsed_sec // 60)}:{int(elapsed_sec % 60):02d} / {int(total_sec // 60)}:{int(total_sec % 60):02d}",
            w,
        )
        self._line(fragments, "-" * w, w)
        ...
        canvas = [" " * w for _ in range(max(0, table_height))]
        handlers: Dict[int, List[Tuple[int, int, object]]] = {}

        def put(row: int, col: int, text: str) -> None:
            if row < 0 or row >= len(canvas) or col >= w:
                return
            col = max(0, col)
            text = str(text)
            available = max(0, w - col)
            line = canvas[row]
            canvas[row] = line[:col] + text[:available] + line[col + min(len(text), available):]

        def set_row_handler(row: int, handler, start_col: int, end_col: int) -> None:
            if 0 <= row < len(canvas):
                handlers.setdefault(row, []).append(
                    (max(0, start_col), min(w, end_col), handler)
                )

        def row_dispatcher(row_handlers: List[Tuple[int, int, object]]):
            def _dispatch(mouse_event):
                x = getattr(mouse_event.position, "x", 0)
                for start_col, end_col, handler in row_handlers:
        ...
        if table_height > 0:
            ...
            with self._lock:
                self.queue_len = len(queue_items)
                self.similar_len = len(similar_entries)
                self.playlist_len = len(playlists)

            most_time_metric: Dict[str, float] = {}
            if self.most_sort_mode == "time" and callable(listen_hours_for_stem):
                for name, cnt in counts.items():
                    most_time_metric[name] = float(listen_hours_for_stem(name, cnt))
                most_entries = sorted(
                    counts.items(),
            ...
            if callable(listen_hours_for_stem):
                most_hours_metric = {
                    name: float(listen_hours_for_stem(name, cnt))
                    for name, cnt in most_entries
                }
                ...
                most_hours_metric = {name: 0.0 for name, _ in most_entries}
            m_count_num_w = max(1, max((len(str(cnt)) for _, cnt in most_entries), default=1))
            m_time_tok_w = max(
                1,
                max(
                    (len(f"({most_hours_metric.get(name, 0.0):.1f}h)") for name, _ in most_entries),
            ...
            with self._lock:
                ...
                if self.most_selected >= self.most_scroll + m_content_h:
                    self.most_scroll = self.most_selected - m_content_h + 1
                self.most_scroll = min(max(0, self.most_scroll), m_max_scroll)
                most_scroll = self.most_scroll
                most_selected = self.most_selected
                most_focus = self.focus_panel == "most"
                most_sort_mode = self.most_sort_mode

            m_count_w = m_count_num_w + 1 + m_time_tok_w
            m_name_w = max(1, left_w - (2 + 3 + 3 + m_count_w + 5))
            m_focus = "[Most]" if most_focus else " Most "
            m_sort_chip = f"[by:{most_sort_mode}]"
            m_title = self._fit(f"{m_focus} {m_sort_chip}", left_w)
            chip_idx = m_title.find(m_sort_chip)
            with self._lock:
                self.most_sort_col_start = left_x + chip_idx if chip_idx >= 0 else 0
                self.most_sort_col_end = self.most_sort_col_start + len(m_sort_chip) if chip_idx >= 0 else 0
                self.most_bounds = (left_x, table_top, left_w, table_height)
            put(0, left_x, m_title)
            set_row_handler(0, self._mouse_handler("most", role="most_title"), left_x, left_x + left_w)
            for row, i in enumerate(range(most_scroll, min(len(most_entries), most_scroll + m_content_h)), start=1):
                name, cnt = most_entries[i]
                sel = ">" if most_focus and i == most_selected else " "
                hours = most_time_metric.get(name, 0.0) if most_sort_mode == "time" else most_hours_metric.get(name, 0.0)
                time_col = f"({hours:.1f}h)"
                count_col = f"{cnt:>{m_count_num_w}} {time_col:>{m_time_tok_w}}"
                row_text = f"{sel} {i+1:>3} | {count_col} | {str(name)[:m_name_w].ljust(m_name_w)}"
                put(row, left_x, row_text[:left_w])
                set_row_handler(row, self._mouse_handler("most", i), left_x, left_x + left_w)
            ...
            with self._lock:
                ...
                if self.similar_selected >= self.similar_scroll + s_content_h:
                    self.similar_scroll = self.similar_selected - s_content_h + 1
                self.similar_scroll = min(self.similar_scroll, s_max_scroll)
                similar_scroll = self.similar_scroll
                similar_selected = self.similar_selected
                similar_focus = self.focus_panel == "similar"
                self.similar_bounds = (right_x, table_top + top_y, right_w, top_h)
            s_title = "[Similar]" if similar_focus else " Similar "
            if similar_mood:
                s_title = (s_title[: max(0, right_w - len(str(similar_mood)) - 3)] + f" ({similar_mood})")[:right_w]
            put(top_y, right_x, self._fit(s_title, right_w))
            set_row_handler(top_y, self._mouse_handler("similar"), right_x, right_x + right_w)
            s_name_w = max(1, right_w - (2 + 3 + 3 + 6 + 5))
            for row, i in enumerate(range(similar_scroll, min(len(entries), similar_scroll + s_content_h)), start=top_y + 1):
                base, score = entries[i]
                sel = ">" if i == similar_selected and similar_focus else " "
                row_text = f"{sel} {i+1:>3} | {str(base)[:s_name_w].ljust(s_name_w)}| {float(score):>6.3f}  "
                put(row, right_x, row_text[:right_w])
                set_row_handler(row, self._mouse_handler("similar", i), right_x, right_x + right_w)
            ...
            with self._lock:
                ...
                if self.queue_selected >= self.queue_scroll + q_content_h:
                    self.queue_scroll = self.queue_selected - q_content_h + 1
                self.queue_scroll = min(self.queue_scroll, q_max_scroll)
                queue_scroll = self.queue_scroll
                queue_selected = self.queue_selected
                queue_focus = self.focus_panel == "queue"
                self.queue_bounds = (right_x, table_top + mid_y, right_w, mid_h)
            q_name_w = max(1, right_w - (2 + 3 + 3 + 4 + 5))
            with self._lock:
                self.queue_name_col_start = right_x + 8
                self.queue_name_col_end = self.queue_name_col_start + q_name_w
                self.queue_mode_col_start = self.queue_name_col_end + 2
                self.queue_mode_col_end = self.queue_mode_col_start + 4
            q_title = "[Queue]" if queue_focus else " Queue "
            put(mid_y, right_x, self._fit(q_title, right_w))
            set_row_handler(mid_y, self._mouse_handler("queue"), right_x, right_x + right_w)
            order = list(range(self.queue_len))
            with self._lock:
                drag_start = self.queue_drag_start
                drag_target = self.queue_drag_target
            if drag_start is not None and drag_target is not None and self.queue_len:
                start = max(0, min(self.queue_len - 1, drag_start))
                target = max(0, min(self.queue_len - 1, drag_target))
                if start != target:
                    item_idx = order.pop(start)
                    order.insert(target, item_idx)
            for row, i in enumerate(range(queue_scroll, min(self.queue_len, queue_scroll + q_content_h)), start=mid_y + 1):
                item_idx = order[i]
                item = queue_items[item_idx]
                path = item.get("path") if isinstance(item, dict) else None
                name = path.name if isinstance(path, Path) else "(unknown)"
                mode = "once" if isinstance(item, dict) and item.get("play_once") else "loop"
                if drag_start is not None and item_idx == drag_start:
                    sel = "D"
                else:
                    sel = ">" if i == queue_selected and queue_focus else " "
                row_text = f"{sel} {i+1:>3} | {name[:q_name_w].ljust(q_name_w)}| {mode:>4}  "
                put(row, right_x, row_text[:right_w])
                set_row_handler(row, self._mouse_handler("queue", i), right_x, right_x + right_w)
            ...
            with self._lock:
                ...
                if self.playlist_selected >= self.playlist_scroll + p_content_h:
                    self.playlist_scroll = self.playlist_selected - p_content_h + 1
                self.playlist_scroll = min(self.playlist_scroll, p_max_scroll)
                playlist_scroll = self.playlist_scroll
                playlist_selected = self.playlist_selected
                playlist_focus = self.focus_panel == "playlists"
                self.playlist_bounds = (right_x, table_top + bot_y, right_w, bot_h)
            p_name_w = max(1, right_w - (2 + 3 + 3 + 4 + 5))
            p_title = "[Playlists]" if playlist_focus else " Playlists "
            put(bot_y, right_x, self._fit(p_title, right_w))
            set_row_handler(bot_y, self._mouse_handler("playlists"), right_x, right_x + right_w)
            for row, i in enumerate(range(playlist_scroll, min(len(playlists), playlist_scroll + p_content_h)), start=bot_y + 1):
                name, count = playlists[i]
                sel = ">" if i == playlist_selected and playlist_focus else " "
                row_text = f"{sel} {i+1:>3} | {str(name)[:p_name_w].ljust(p_name_w)}| {int(count):>4}  "
                put(row, right_x, row_text[:right_w])
                set_row_handler(row, self._mouse_handler("playlists", i), right_x, right_x + right_w)

            for i in range(table_height):
                put(i, left_w, "│")
            if right_w > 0 and top_h > 0 and mid_h > 0:
                put(sep1_y, right_x, "─" * right_w)
            if right_w > 0 and mid_h > 0 and bot_h > 0:
                put(sep2_y, right_x, "─" * right_w)

        for idx, line in enumerate(canvas):
            row_handlers = handlers.get(idx)
            self._line(
                fragments,
                line,
                w,
                handler=row_dispatcher(row_handlers) if row_handlers else None,
            )

        status = self.status_msg or ""
        if status:
            self._line(fragments, (" " * max(0, w - 2 - len(status))) + status, w, style="class:status")
        else:
            self._line(fragments, "", w)
        self._line(fragments, "-" * w, w)
        prompt = "mood: " if self.input_mode == "mood" else "playlist name: "
        if not self.input_buffer and not now_playing and self.input_mode == "mood":
            self._line(fragments, f"  {prompt}(type a mood + Enter; optional: (top N) / top=N)", w)
        else:
            self._line(fragments, f"  {prompt}{self.input_buffer}", w)
        self._line(fragments, "  Enter submit/load • TAB focus • Ctrl+S save • Ctrl+G stop • --help", w)
        ...
        if self.show_help:
            help_lines = [
                ...
                "Mood directives: top=5, vol=-16, speaker on/off, speaker offset=85ms",
                "TAB: focus (Queue/Similar/Playlists/Most)",
                "Up/Down PgUp/PgDn: move in focused panel",
                "Right: skip track",
                "Ctrl+G: stop",
                "Enter: add selected (Similar)",
                "Drag in Queue: reorder",
            ...
            max_line = max(len(line) for line in help_lines) if help_lines else 0
            fragments.append(("class:help", "\n" + self._fit(" Shortcuts ".center(min(w, max(30, max_line + 4)), "-"), min(w, max(30, max_line + 4))) + "\n"))
            for line in help_lines[2:]:
                fragments.append(("class:help", self._fit(f"  {line}", min(w, max(30, max_line + 4))) + "\n"))
    ...
    def render(
        ...
        queue_items = queue_items or []
        similar_entries = similar_entries or []
        playlists = playlists or []
        with self._lock:
            self.queue_len = len(queue_items)
            self.similar_len = len(similar_entries)
            self.playlist_len = len(playlists)
            self._consume_pending_wheel()
            self._snapshot = {
                ...
                "speaker_offset_sec": speaker_offset_sec,
                "elapsed_sec": elapsed_sec,
                "total_sec": total_sec,
                "playback_mode": playback_mode,
                "counts": dict(counts or {}),
                "listen_hours_for_stem": listen_hours_for_stem,
                "similar_entries": list(similar_entries),
        ...
        try:
            submitted = self._submitted.get_nowait()
        except queue_mod.Empty:
            submitted = None

        if submitted and submitted.strip() != "--help":
            self.status_msg = f"Submitted: “{submitted}”"
        return submitted
...
class MacSpeakerMirror:
    ...
    def available(self) -> bool:
        return (
            sys.platform == "darwin"
            and AVAudioPlayer is not None
            and NSURL is not None
        )
    ...
    def set_volume(self, volume: float) -> bool:
        if self.player is None:
            return False
        try:
            self.player.setVolume_(max(0.0, min(1.0, float(volume))))
            return True
        except Exception:
            return False

    def _build_player(
        self, track_path: Path, position_sec: float, volume: float
    ) -> object | None:
        if not self.available:
            return None
        try:
            url = NSURL.fileURLWithPath_(str(track_path.resolve()))
            ...
            if duration > 0.05:
                pos = min(pos, max(0.0, duration - 0.05))
            try:
                player.setCurrentTime_(pos)
            except Exception:
                pass
    ...
    def start(
        ...
        volume: float = BASE_VOLUME_SCALE,
        schedule_delay_sec: Optional[float] = None,
    ) -> bool:
        self.stop()
        player = self._build_player(track_path, position_sec, volume)
        if player is None:
            return False
        try:
            ok = False
            if (
                schedule_delay_sec is not None
                and float(schedule_delay_sec) > 0.0
                and hasattr(player, "deviceCurrentTime")
                and hasattr(player, "playAtTime_")
            ):
                target = float(player.deviceCurrentTime()) + float(schedule_delay_sec)
                ok = bool(player.playAtTime_(target))
            else:
                ok = bool(player.play())
            if not ok:
                return False
...
def estimate_track_speaker_offset_sec(track_path: Path, base_offset_sec: float) -> float:
    # Small heuristic only. The goal is to bias startup for files whose decode/seek
    # path tends to vary more, not to predict true hardware latency.
    offset = float(base_offset_sec)
    try:
        info = MP3(str(track_path)).info
        bitrate = float(getattr(info, "bitrate", 0.0) or 0.0)
        sample_rate = int(getattr(info, "sample_rate", 0) or 0)
        bitrate_mode = str(getattr(info, "bitrate_mode", "") or "")

        if "VBR" in bitrate_mode:
            offset += 0.018
        elif "CBR" in bitrate_mode:
            offset -= 0.004

        if bitrate and bitrate < 160000:
            offset += 0.008
    ...
    return clamp_float(
        offset,
        MIN_SPEAKER_MIRROR_OFFSET_SEC,
        MAX_SPEAKER_MIRROR_OFFSET_SEC,
    )
...
def main(
    ...
    speaker_mirror_enabled = False
    speaker_mirror_warned = False
    speaker_mirror_offset_sec = DEFAULT_SPEAKER_MIRROR_OFFSET_SEC
    speaker_mirror = MacSpeakerMirror()
    ...
    def stop_speaker_mirror() -> None:
        speaker_mirror.stop()

    def sync_speaker_mirror(
        current_track: Optional[Path],
        position_sec: float = 0.0,
        volume_scale: float = BASE_VOLUME_SCALE,
        tui: Optional[CursesTUI] = None,
        force_restart: bool = False,
    ) -> None:
        nonlocal speaker_mirror_warned
        if not speaker_mirror_enabled or current_track is None:
            stop_speaker_mirror()
            return
        if not speaker_mirror.available:
            stop_speaker_mirror()
            if not speaker_mirror_warned and tui and tui.enable:
                tui.status_msg = "Speaker mirror unavailable on this system."
                speaker_mirror_warned = True
            return
        if (
            not force_restart
            and
            speaker_mirror.track_path == current_track
            and speaker_mirror.is_playing()
        ):
            speaker_mirror.set_volume(volume_scale)
            return
        effective_position_sec = max(
            0.0,
            float(position_sec) + float(speaker_mirror_offset_sec),
        )
        ok = speaker_mirror.start(
            current_track,
            position_sec=effective_position_sec,
            volume=volume_scale,
        )
        if not ok and tui and tui.enable:
            tui.status_msg = "Failed to start speaker mirror."

    def start_track_outputs(
        track_path: Path,
        sound: "pygame.mixer.Sound",
        volume_scale: float,
        tui: Optional[CursesTUI],
    ) -> Optional["pygame.mixer.Channel"]:
        track_offset_sec = estimate_track_speaker_offset_sec(
            track_path, speaker_mirror_offset_sec
        )
        pygame_delay_sec = 0.0

        if speaker_mirror_enabled:
            mirror_position_sec = max(0.0, track_offset_sec)
            if track_offset_sec < 0.0:
                pygame_delay_sec = abs(track_offset_sec)
            mirror_ok = speaker_mirror.start(
                track_path,
                position_sec=mirror_position_sec,
                volume=volume_scale,
                schedule_delay_sec=SPEAKER_MIRROR_PREROLL_SEC,
            )
            if not mirror_ok and tui and tui.enable:
                tui.status_msg = "Failed to start speaker mirror."

        if speaker_mirror_enabled:
            target_ts = (
                time.monotonic()
                + SPEAKER_MIRROR_PREROLL_SEC
                + pygame_delay_sec
            )
            while True:
                remaining = target_ts - time.monotonic()
                if remaining <= 0.0:
                    break
        ...
        if tui and tui.enable and speaker_mirror_enabled:
            tui.status_msg = (
                f"Speaker mirror sync: base {speaker_mirror_offset_sec * 1000.0:+.0f}ms,"
                f" track {track_offset_sec * 1000.0:+.0f}ms"
            )
        return channel

    def set_speaker_mirror_offset(
        offset_sec: float,
        tui: Optional[CursesTUI],
        current_track: Optional[Path] = None,
        current_elapsed_sec: float = 0.0,
        current_volume_scale: float = BASE_VOLUME_SCALE,
    ) -> None:
        nonlocal speaker_mirror_offset_sec
        speaker_mirror_offset_sec = clamp_float(
            offset_sec,
            MIN_SPEAKER_MIRROR_OFFSET_SEC,
            MAX_SPEAKER_MIRROR_OFFSET_SEC,
        )
        if speaker_mirror_enabled and current_track is not None:
            sync_speaker_mirror(
                current_track,
                position_sec=current_elapsed_sec,
                volume_scale=current_volume_scale,
                tui=tui,
                force_restart=True,
            )
        if tui and tui.enable:
            tui.status_msg = (
                f"Speaker mirror offset set to {speaker_mirror_offset_sec * 1000.0:+.0f}ms"
            )

    def set_speaker_mirror_enabled(
        enabled: bool,
        tui: Optional[CursesTUI],
        current_track: Optional[Path] = None,
        current_elapsed_sec: float = 0.0,
        ...
        nonlocal speaker_mirror_enabled
        speaker_mirror_enabled = bool(enabled)
        if speaker_mirror_enabled:
            sync_speaker_mirror(
                current_track,
                position_sec=current_elapsed_sec,
            ...
            if (
                tui
                and tui.enable
                and (current_track is None or speaker_mirror.is_playing())
            ):
                tui.status_msg = "Speaker mirror enabled."
        else:
            stop_speaker_mirror()
            if tui and tui.enable:
                tui.status_msg = "Speaker mirror disabled."
    ...
    def apply_submission(
        ...
        if text.strip() == "--help":
            if tui and tui.enable:
                tui.show_help = True
            return False
        mood_text, new_top, new_vol, speaker_toggle, speaker_offset = parse_mood_and_directives(text)
        restart_for_speaker_toggle = False

        current_scale = (
            float(audio_data[current_track].get("scale") or 0.5)
            if current_track and current_track in audio_data
            else BASE_VOLUME_SCALE
        )

        if speaker_offset is not None:
            set_speaker_mirror_offset(
                speaker_offset,
                tui,
                current_track=current_track,
                current_elapsed_sec=current_elapsed_sec,
                current_volume_scale=current_scale,
            )
        ...
        if speaker_toggle is not None:
            was_enabled = speaker_mirror_enabled
            set_speaker_mirror_enabled(
                speaker_toggle,
                tui,
                current_track=current_track,
                current_elapsed_sec=current_elapsed_sec,
            ...
            if current_track is not None and speaker_toggle and not was_enabled:
                restart_for_speaker_toggle = True
                if tui and tui.enable:
                    tui.status_msg = "Speaker mirror enabled. Restarting current track from the beginning."
        ...
        return restart_for_speaker_toggle
    ...
    tui_cls = PromptToolkitTUI if tui_backend == "prompt" else CursesTUI
    with tui_cls(enable=enable_tui) as tui:
        ...
        def pause_silence(seconds: float) -> tuple[bool, float | None]:
            ...
            while True:
                ...
                submitted = tui.render(
                    ...
                    loudness_diff=None,
                    volume_scale=None,
                    speaker_enabled=speaker_mirror_enabled,
                    speaker_offset_sec=speaker_mirror_offset_sec,
            ...
            while True:
                ...
                while True:
                    ...
                    try:
                        ...
                        if channel is None:
                            raise pygame.error("No audio channel available")
                            ...
                            if enable_tui:
                                ...
                                if submitted:
                                    if not handle_save_playlist_input(submitted):
                                        restart_current_track = apply_submission(
                                            submitted,
                                            tui,
                                            track_path,
                                            channel,
                        ...
                        if restart_current_track:
                            current_playing_item = current_item
                            continue
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        default="prompt",
        help="Terminal UI backend. The copied script defaults to prompt_toolkit.",
    )
    return p.parse_args()
...
if __name__ == "__main__":
    ...
    try:
        main(
            ...
            tui_backend=str(args.tui_backend),
        )
    except SystemExit:
        print("\nPlayback stopped.")
