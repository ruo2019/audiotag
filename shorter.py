...
# --- SwitchAudioSource helpers (requires: brew install switchaudio-osx) ---

def _ensure_switchaudiosource() -> None:
    if sys.platform == "darwin" and shutil.which("SwitchAudioSource") is None:
        raise RuntimeError(
            "SwitchAudioSource not found. Install it with:\n"
            "  brew install switchaudio-osx\n"
            "Refusing to play to avoid speaker fallback."
        )

def _run_cmd(cmd: List[str]) -> str:
    return subprocess.check_output(cmd, stderr=subprocess.STDOUT, text=True).strip()

def get_macos_output_device() -> Optional[str]:
    try:
        return _run_cmd(["SwitchAudioSource", "-c", "-t", "output"])
    except Exception:
        return None

def set_macos_output_device(device_name: str) -> bool:
    try:
        _run_cmd(["SwitchAudioSource", "-s", device_name, "-t", "output"])
        return True
    except Exception:
        return False

def list_macos_output_devices() -> List[str]:
    try:
        out = _run_cmd(["SwitchAudioSource", "-a", "-t", "output"])
        return [line.strip() for line in out.splitlines() if line.strip()]
    except Exception:
        return []

def start_coreaudio_watchdog(
    exit_event: threading.Event,
    on_stop: Callable[[], None],
    allowed_substring: Optional[str] = None,
    disallow_speakers: bool = True,
    poll_interval: float = 0.25,
    logging: bool = True,
) -> threading.Thread:
    """
    Watches macOS *system* output device.
    - If output becomes 'MacBook Speakers' -> stop immediately.
    - If allowed_substring is set -> stop if output device name does NOT contain it.
    Fail-closed: if we can't read output device, we stop playback and exit.
    """
    def _thread():
        ...
        while not exit_event.is_set():
            ...
            current = get_macos_output_device()
            if current is None:
                if logging:
                    print("[audio] Can't read macOS output device. Stopping to avoid speaker playback.")
                on_stop()
                exit_event.set()
                return
            ...
            if disallow_speakers and "macbook speakers" in cur_l:
                if logging:
                    print("[audio] Output switched to MacBook Speakers. Stopping to avoid speaker playback.")
                on_stop()
                exit_event.set()
                return

            if allowed_substring and (allowed_substring.lower() not in cur_l):
                if logging:
                    print(f"[audio] Output changed away from '{allowed_substring}'. Current: '{current}'. Stopping.")
                on_stop()
                exit_event.set()
                return

    t = threading.Thread(target=_thread, name="CoreAudioWatchdog", daemon=True)
    t.start()
    return t

def stop_playback_safely() -> None:
    try:
...
def main(
    ...
    if sys.platform == "darwin":
        _ensure_switchaudiosource()

        FORCE_DEVICE = "External Headphones"  # must match SwitchAudioSource -a -t output exactly

        available = list_macos_output_devices()
        if FORCE_DEVICE not in available:
            raise RuntimeError(
                f"Output device '{FORCE_DEVICE}' not found.\n"
                f"Available outputs: {available}\n"
                "Pick one of the exact names above."
            )

        if not set_macos_output_device(FORCE_DEVICE):
            raise RuntimeError(f"Could not switch macOS output to '{FORCE_DEVICE}'. Refusing to play.")

        time.sleep(0.2)

        cur = get_macos_output_device()
        if not cur or FORCE_DEVICE.lower() not in cur.lower():
            raise RuntimeError(
                f"Attempted to switch output to '{FORCE_DEVICE}', but current output is '{cur}'. Refusing to play."
            )

    # initialize pygame AFTER the output is forced
    if logging:
        print("Initializing pygame mixer...")
    init_pygame()

    # watchdog can start after init (stop_playback_safely will work either way, but this is clean)
    if sys.platform == "darwin":
        start_coreaudio_watchdog(
            exit_event=EXIT_NOW,
            on_stop=stop_playback_safely,
            allowed_substring=FORCE_DEVICE,
            disallow_speakers=True,
            poll_interval=0.25,
            logging=logging,
        )
    else:
        # Not macOS: no SwitchAudioSource enforcement available
        if logging:
            print("[audio] CoreAudio watchdog not enabled (not macOS).")
